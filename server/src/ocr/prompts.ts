...
- A charge is an adjustment only if adding it to the item subtotal gets you closer to the total actually due. It is NOT an adjustment if it is merely a breakdown/restatement of an amount already fully included elsewhere — including it as an adjustment as well would double-count it. This is a common, specific pattern on Philippine BIR-format receipts: a block near the bottom listing lines like "GROSS AMOUNT"/"GROSS SALES", "VATABLE", "VAT EXEMPT", "ZERO RATED Sales", "VAT Amount", and "Net Sale" is restating how the ALREADY-CHARGED item total breaks down into its gross figure and its VAT-exclusive base plus VAT component — none of these are adjustments, even though "VAT Amount" looks like a tax and "Gross Amount"/"Gross Sales" looks like a total. A concrete tell: if item unit prices are printed with a trailing "v" or "VAT-inc"-style marker (e.g. "85.00v"), those item prices are already VAT-inclusive, so a separate "VAT Amount" line is just showing you the VAT already baked into the items you extracted — do not add it again. Likewise, if a charge (e.g. "Service Charge") is printed more than once (once in the main total block, again inside this recap block), include it exactly once, not twice.
  Worked example — a receipt printing:
    ...
    VAT AMOUNT:                   138.75
  extracts as items [{"name":"Half Bulalo","quantity":1,"lineTotalCentavos":53000}, {"name":"Ribs Set","quantity":1,"lineTotalCentavos":28000}, ...the rest of the item table...], adjustments: [] (empty — "GROSS AMOUNT" restates the item total, "VATABLE SALES"/"VAT AMOUNT" restate that same total's VAT-exclusive/VAT split; none is a charge on top), and detectedTotalCentavos: 129500. Do NOT emit an adjustment for "VAT AMOUNT" here, and do NOT set detectedSubtotalCentavos to 115625 (VATABLE SALES) — see the next rule.
- "detectedSubtotalCentavos" must be consistent with the item prices you actually extracted, never with a recap-block figure computed on a different basis. Item unit prices on these receipts are ordinarily VAT-inclusive (they sum to the VAT-inclusive total), so "VATABLE SALES" — the VAT-EXCLUSIVE base from the recap block — is almost never the right value for it (using it would silently disagree with your own item sum, exactly like a phantom double-counted adjustment would). If you cannot identify a distinct, genuine subtotal line that is on the same VAT-inclusive basis as your items, set detectedSubtotalCentavos to null rather than guessing from the VAT breakdown block.
